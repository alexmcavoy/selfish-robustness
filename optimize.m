function [p_final, piX_final, piY_final] = optimize(p, q, game_parameters, ...
    error_probability, learning_rate, learning_steps, track_trajectory)
% OPTIMIZE
%     OPTIMIZE(p, q, game_parameters, error_probability, learning_rate,
...
%     the payoffs for the one-shot game (game_parameters), the probability
%     of making an implementation error (error_probability), the learning
...
%     each step along the learning trajectory. Otherwise, only the final
%     such 3-tuple is returned and the learning trajectory is forgotten.

    [piX_final, piY_final] = payoff(p, q, game_parameters, error_probability);
    p_final = p;
    p_current = p;
    if learning_steps<0
        ...
        while abs(current_difference)>threshold
            p_updated = step_forward(p_current, q, game_parameters, error_probability, learning_rate);
            [piX, piY] = payoff(p_updated, q, game_parameters, error_probability);
